...
import { createDiagnosisReport } from '../lib/diagnosis'
        ...
        <aside className="content-panel ops-stack">
          ...
          {topSignal ? (
            <article className="signal-card signal-card-danger">
              ...
              <div className="form-actions">
                <span className="pill">{priorityLabel(topSignal.priority)}</span>
                {topSignal.href ? (
                  <Link className="secondary-action" to={topSignal.href}>
                    前往处理
                  </Link>
                ) : null}
              </div>
            ...
            {report.signals.map((signal) => (
              <article key={signal.title} className="activity-group">
                <div className="activity-group-header">
                  ...
                  <span className="pill">{priorityLabel(signal.priority)}</span>
                </div>
                <p>{signal.detail}</p>
                {signal.href ? (
                  <div className="card-actions">
                    <Link className="inline-action" to={signal.href}>
                      去处理
                    </Link>
                  </div>
                ) : null}
        ...
        <aside className="content-panel">
          ...
          <div className="portfolio-rebalance-list">
            {report.actions.map((action, index) => (
              <article key={action.title} className="setting-card portfolio-rebalance-item">
                <div className="portfolio-rebalance-top">
                  ...
                  <span className="pill">{priorityLabel(action.priority)}</span>
                </div>
                <p>{action.detail}</p>
                <div className="card-actions">
                  <Link className="inline-action" to={action.href}>
                    打开对应模块
                  </Link>
                </div>
              </article>
