...
export function createDiagnosisReport(data: HouseholdData): DiagnosisReport {
  ...
  if (metrics.liabilityRatio > 50) {
    signals.push({
      ...
      priority: 'high',
      href: '/liabilities?sort=amount-desc',
    })
  }
  ...
  if (positions.concentrationRatio > 35) {
    signals.push({
      ...
      href: '/portfolio?sortKey=allocationRatio&sortDirection=desc',
    })
  }
  ...
  if (Math.abs(positions.totalTargetWeight - 100) > 5) {
    signals.push({
      ...
      priority: 'medium',
      href: '/portfolio?sortKey=targetWeightDrift&sortDirection=desc',
    })
  }
